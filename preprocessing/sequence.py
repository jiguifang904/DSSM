# -*- coding: utf-8 -*-
"""
Created on Thu Sep 30 18:13:30 2021
...
class SequencePoolingLayer(nn.Module):    # 稀疏边长特征进行pooling操作
    ...
    def _sequence_mask(self, lengths, maxlen=None, dtype=torch.bool):
        ...
        mask = row_vector < matrix   # [False,  True,...]满足条件为True，否则为False
    ...
    def forward(self, seq_value_len_list):
        ...
        # 复制tensor， torch.repeat_interleave(tensor，复制次数， 维度)
...
class AttentionSequencePoolingLayer(nn.Module):
    def __init__(self, att_hidden_units=(80, 40), att_activation='sigmoid', weight_normalization=False,
        ...
        self.local_att = LocalActivationUnit(hidden_units=att_hidden_units, embedding_dim=embedding_dim,
                                             activation=att_activation, dropout_rate=0, 
                                             use_bn=False)   # use_bn是否使用batchnorm
    ...
    def forward(self, query, keys, keys_length, mask=None):
        ...
        # torch.where()函数的作用是按照一定的规则合并两个tensor类型, 合并outputs, padding
...
if __name__ == "__main__":
    row_vector = torch.arange(0, 10, 1)
    matrix =  torch.arange(0, 20, 2)
    
    print(row_vector)
    mask = row_vector < matrix
    print(mask)
    print(1, mask.type(torch.bool))
    row_vector = row_vector.unsqueeze(1)
    print(row_vector.size())
    padding = torch.ones_like(row_vector)
    print(padding)
    print(torch.where(mask, row_vector, padding))