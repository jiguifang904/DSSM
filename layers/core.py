# -*- coding: utf-8 -*-
"""
Created on Thu Sep 30 17:42:06 2021
...
from layers.activation import activation_layer  # 激活函数选择


class PredictionLayer(nn.Module):   # 预测类
...
class DNN(nn.Module):  # 多层全连接（激活函数，batch_normld)
    def __init__(self, inputs_dim, hidden_units, activation='relu', l2_reg=0, dropout_rate=0, use_bn=False,
        ...
        self.seed = seed
        self.l2_reg = l2_reg
        self.use_bn = use_bn  # 是否用batch_normld层
        if len(hidden_units) == 0:
            raise ValueError("hidden_units is empty!!")
            
        hidden_units = [inputs_dim] + list(hidden_units)  # 多层全连接
...
class LocalActivationUnit(nn.Module):
    def __init__(self, hidden_units=(64, 32), embedding_dim=4, activation='sigmoid', dropout_rate=0,
        ...
        self.dense = nn.Linear(hidden_units[-1], 1)    # 最后一层全连接
